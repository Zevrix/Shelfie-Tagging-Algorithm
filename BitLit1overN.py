...
# BISACs contributed 1/n to the count here instead of 1, n = the number of BISACs in the specific book we are dealing with
# this is to prevent books with lots of tags crowding out those with less
...
def get_x(a,b,c):
    ...
    for x in L9:
        for y in x:
            if y[:3] == a:
                acount = acount + 1/len(x)
    bcount = 0
    for x in L9:
        for y in x:
            if y[:6] == b:
                bcount = bcount + 1/len(x)
    ccount = 0
    for x in L9:
        for y in x:
            if y == c:
                ccount = ccount + 1/len(x)
    a2count = 0
    for x in L12:
        for y in x:
            if y[:3] == a:
                a2count = a2count + 1/len(x)
    b2count = 0
    for x in L12:
        for y in x:
            if y[:6] == b:
                b2count = b2count + 1/len(x)
    c2count = 0
    for x in L12:
        for y in x:
            if y == c:
                c2count = c2count + 1/len(x)
    if b[3:6] == '000':
        bcount = 0
        b2count = 0
    if c[6:] == '000':
...
L13.reverse()
print()
print("Experiment 2: 1/n Count")
with open("Results.txt", "a") as text_file:
    print(file = text_file)
    print("Experiment 2: 1/n Count", file = text_file)
    print(file = text_file)
print()
for x in L13:
    with open("Results.txt", "a") as text_file: 
