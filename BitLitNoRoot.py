...
# Because the database counts are not being squarerooted, there is a lot more value assigned to uncommon tags
...
while end != len(L10):
    for y in L10[end]:
        ...
        for k in L11:
            if k[1] == b:
                l = (1/n)*(scores[0]+cst*scores[1])
                m = (1/o)*(scores[2]+cst*scores[3])
                q = (1/p)*(scores[4]+cst*scores[5])
                r = 1
                if b[6:] != '000':
                    r = 4
                elif b[3:6] != '000':
...
L13.reverse()
print()
print("Experiment 1: No Root")
with open("Results.txt", "a") as text_file:
    print(file = text_file)
    print("Experiment 1: No Root", file = text_file)
    print(file = text_file)
print()
for x in L13:
    with open("Results.txt", "a") as text_file: 
